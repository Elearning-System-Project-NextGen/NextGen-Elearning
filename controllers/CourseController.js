...
class CourseController {
  static async getCourse(req, res) {
    const course = await Course.findById(req.params.id).populate('subject_id teacher_id');
    if (!course) {
  ...
  static async createCourse(req, res) {
    ...
    const { subject_id, title_en, title_ar, description_en, description_ar } = req.body;
    if (!subject_id || !title_en) {
